...
## ** 1. Change the tier rounding adjustments ** ##
## ** 2. Change in weights by Business Unit ** ##
## ** 3. Exculde 0 brand market writer from deciling ** ##
...
colSums(is.na(digital_bag))
summary(digital_bag)
fwrite(digital_bag,"Chi Cheng/Master Digital Bag/Processed Data/digital_bag_pre_scoring_0514.csv",row.names = F)
...
weights_pih <- c(0.5, # 1.brand_trx_dol
             0.25, # 2.market relevance
             0.1, # 3.headroom (absolute value)
             0.1, # 4.persuadabiity
             0.05) # 5.overall_details (adjusted)

weights_peh <- c(0.517, # 1.brand_trx_dol
                 0.25, # 2.market relevance
                 0.1, # 3.headroom (absolute value)
                 0.1, # 4.persuadabiity
                 0.033) # 5.overall_details (adjusted)

weights_c <- c(0.525, # 1.brand_trx_dol
                 0.25, # 2.market relevance
                 0.1, # 3.headroom (absolute value)
                 0.1, # 4.persuadabiity
                 0.025) # 5.overall_details (adjusted)
...
digital_bag <- digital_bag %>% 
  mutate(
    ...
    overall_score_nor = ifelse(brand_trx_dol==0 & persuadability==0 & headroom_abs==0,0,
                               ifelse(Bu == "PIH",
                                      weights_pih[1]* brand_trx_dol_nor + weights_pih[2]*mrkt_rel_nor 
                                      + weights_pih[3]*headroom_abs_nor + weights_pih[4]*persuadability_nor + weights_pih[5]*overall_details_adj_nor
                                      ,
                                      ifelse(Bu == "PEH",
                                             weights_peh[1]* brand_trx_dol_nor + weights_peh[2]*mrkt_rel_nor 
                                             + weights_peh[3]*headroom_abs_nor + weights_peh[4]*persuadability_nor + weights_peh[5]*overall_details_adj_nor
                                             ,
                                             ifelse(Bu == "Consumer",
                                                    weights_c[1]* brand_trx_dol_nor + weights_c[2]*mrkt_rel_nor 
                                                    + weights_c[3]*headroom_abs_nor + weights_c[4]*persuadability_nor + weights_c[5]*overall_details_adj_nor
                                                    ,NA)))))

# Get summary of the scored dataset
summary(digital_bag)
# Save a copy of output
fwrite(digital_bag,"Chi Cheng/Master Digital Bag/Processed Data/digital_bag_scored_0517.csv",row.names = F)
...
# Get bottom 10th percentile brand_trx cutoff by brand
digital_bag_email_brand_10 <- digital_bag_email[digital_bag_email$brand_trx>0,] %>%
  group_by(brand) %>%
  dplyr::summarize(
    cutoff_brand_10 = quantile(brand_trx,probs=0.1)
  )

digital_bag_email <- merge(digital_bag_email, digital_bag_email_brand_10, by="brand", all.x=T)
digital_bag_email[is.na(digital_bag_email$cutoff_brand_10),]$cutoff_brand_10 <- 0

# Get bottom 90th percentile brand_trx cutoff by brand
digital_bag_email_brand_90 <- digital_bag_email[digital_bag_email$brand_trx>0,] %>%
  group_by(brand) %>%
  dplyr::summarize(
    cutoff_brand_90 = quantile(brand_trx,probs=0.9)
  )

digital_bag_email <- merge(digital_bag_email, digital_bag_email_brand_90, by="brand", all.x=T)
digital_bag_email[is.na(digital_bag_email$cutoff_brand_90),]$cutoff_brand_90 <- 0


# Get 50th percentile mrkt_trx cutoff by brand
digital_bag_email_mrkt_50 <- digital_bag_email[digital_bag_email$mrkt_trx>0,] %>%
  group_by(brand) %>%
  dplyr::summarize(
    cutoff_mrkt_50 = quantile(mrkt_trx,probs=0.5)
  )

digital_bag_email<- merge(digital_bag_email, digital_bag_email_mrkt_50, by="brand", all.x=T)
digital_bag_email[is.na(digital_bag_email$cutoff_mrkt_50),]$cutoff_mrkt_50 <- 0


# Add an indicator whether a HCP received any email in 2016
brand_adj <- unique(digital_bag_email[!is.na(digital_bag_email$brand_trx_dol_add),]$brand)
digital_bag_email$emails_dlvd_2016_ind <- ifelse((digital_bag_email$emails_dlvd_2016_hcp_brand>0 & 
                                                    (digital_bag_email$brand_trx >= digital_bag_email$cutoff_brand_10 | 
                                                       digital_bag_email$mrkt_trx >= digital_bag_email$cutoff_mrkt_50 |
                                                 ...
                                                 | digital_bag_email$brand=="XTANDI" |
                                                   (digital_bag_email$emails_dlvd_2016_hcp_brand==0 &
                                                      (digital_bag_email$brand_trx >= digital_bag_email$cutoff_brand_90 | 
...
# Embeda, Ibrance, Torisel, Protonix, Sutent, Xalkori, Xtandi Bosulif, Inlyta, Aromasin, Estring
...
table(digital_bag_email$digital_bag_ind) # 4.5M fall into digital bag out of 28.6M combos
...
fwrite(digital_bag_email_10,"Chi Cheng/Master Digital Bag/Processed Data/digital_bag_output_business_rule_0515.csv",row.names = F)
...
# Save output
fwrite(digital_bag_email_10,"Chi Cheng/Master Digital Bag/Output Data/digital_bag_email_distribution_top10_0517.csv",row.names = F)
